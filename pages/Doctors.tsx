...
const Doctors: React.FC = () => {
    ...
    const getStatusChip = (status: 'نشط' | 'غير نشط') => {
        const color = status === 'نشط' ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800';
                ...
                <table className="w-full text-right">
                    <thead className="bg-gray-100">
                        <tr>
                            ...
                            <th className="p-3 text-sm font-semibold tracking-wide">التخصص</th>
                            ...
                            <th className="p-3 text-sm font-semibold tracking-wide">البريد الإلكتروني</th>
                    ...
                    <tbody className="divide-y divide-gray-100">
                        {doctors.map(doctor => (
                            <tr key={doctor.doctor_id} className="hover:bg-gray-50">
                                <td className="p-3 text-sm text-gray-700">{doctor.doctor_id}</td>
                                <td className="p-3 text-sm text-gray-700 font-medium">{doctor.doctor_name}</td>
                                <td className="p-3 text-sm text-gray-700">{doctor.specialty}</td>
                                <td className="p-3 text-sm text-gray-700">{getClinicName(doctor.clinic_id)}</td>
                                <td className="p-3 text-sm text-gray-700">{doctor.phone}</td>
                                <td className="p-3 text-sm text-gray-700">{doctor.email}</td>
                                <td className="p-3 text-sm text-gray-700">{doctor.shift}</td>
                                <td className="p-3 text-sm text-gray-700">{getStatusChip(doctor.status)}</td>
                            </tr>
                        ))}
                    </tbody>
                </table>
                ...
                <form onSubmit={handleSubmit} className="space-y-4">
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                            ...
                            <input type="text" name="doctor_name" value={formData.doctor_name} onChange={handleChange} className="w-full p-2 border rounded" required />
                        </div>
                        <div>
                            <label className="block text-sm font-medium text-gray-700 mb-1">التخصص</label>
                            <input type="text" name="specialty" value={formData.specialty} onChange={handleChange} className="w-full p-2 border rounded" required />
                        </div>
                         <div>
                        ...
                        <div>
                         ...
                         <div>
                            ...
                            <select name="clinic_id" value={formData.clinic_id} onChange={handleChange} className="w-full p-2 border rounded bg-white" required>
                                {clinics.map(c => <option key={c.clinic_id} value={c.clinic_id}>{c.clinic_name}</option>)}
                            </select>
                        </div>
                         <div>
                            <label className="block text-sm font-medium text-gray-700 mb-1">الدوام</label>
                            <select name="shift" value={formData.shift} onChange={handleChange} className="w-full p-2 border rounded bg-white" required>
