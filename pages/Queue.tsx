...
const Queue: React.FC = () => {
    ...
    return (
        <div>
            <h1 className="text-4xl font-bold text-center text-gray-800 mb-8">شاشة الانتظار</h1>
            {Object.keys(visitsByClinic).length === 0 ? (
                <div className="text-center text-gray-500 text-xl bg-white p-10 rounded-lg shadow-md">
                    <p>لا يوجد مرضى في قائمة الانتظار حالياً.</p>
                </div>
            ) : (
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                    {Object.entries(visitsByClinic).map(([clinicName, clinicVisits]) => (
                        <div key={clinicName} className="bg-white p-6 rounded-xl shadow-lg">
                            <h2 className="text-2xl font-bold text-teal-700 mb-4 border-b-2 pb-2 border-teal-200">{clinicName}</h2>
                            <ul className="space-y-4">
                                {clinicVisits.map((visit) => (
                                    <li key={visit.visit_id} className={`p-4 rounded-lg flex items-center justify-between ${visit.status === VisitStatus.InProgress ? 'bg-blue-100 border-l-4 border-blue-500' : 'bg-gray-50'}`}>
                                        <div>
                                            <p className="text-2xl font-bold text-gray-800">{visit.queue_number}</p>
                                            <p className="text-lg text-gray-600">{getPatientName(visit.patient_id)}</p>
                                        </div>
                                        <span className={`px-3 py-1 text-sm font-semibold rounded-full ${visit.status === VisitStatus.InProgress ? 'bg-blue-200 text-blue-800' : 'bg-yellow-200 text-yellow-800'}`}>
                                            {visit.status}
                                        </span>
                                    </li>
                                ))}
